...
describe("ERC1155", () => {
    ...
    it("Create ERC-721 contract", async function () {
        ...
        await factory.connect(alice).createERC721("TEST", "TEST")

        const erc721Address = await factory.nfts(1)

        const erc721 = await ethers.getContractAt('ERC721Template', erc721Address)

        await erc721.connect(alice).mint(alice.address, 1)

        await erc721.connect(alice).setTokenURI(1, "https://api.cryptokitties.co/kitties/1")
