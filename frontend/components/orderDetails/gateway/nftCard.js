...
import CrossChainSwapModal from "../../modals/crosschainSwapModal"
...
const NFTCard = ({
  ...
  const { resolveMetadata, resolveTokenValue, swap, partialSwap } = useOrder();
  const [data, setData] = useState();
  const [loading, setLoading] = useState();

  const [swapModalVisible, setSwapModalVisible] = useState(false);
  const [crosschainSwapModalVisible, setCrosschainSwapModalVisible] = useState(false)
  ...
  const onApprove = useCallback(async (isGateway = false) => {
    setLoading(true);

    try {
      if (item.tokenType === 0) {
        const { approve } = contractErc20;
        await approve(isGateway);
      } else if (item.tokenType === 1) {
        await assetAddressContractErc721.approve(isGateway);
      } else if (item.tokenType === 2) {
        await assetAddressContractErc1155.approve(isGateway, item.assetTokenIdOrAmount);
      }
      setApproval(true)
    } catch (e) {
  ...
  const onPartialSwap = useCallback(async () => {
    ...
    try {
      ...
      const tx = await partialSwap(orderId, order, index);
      await tx.wait();

      setCrosschainSwapModalVisible(false);
    ...
    setLoading(false);
  }, [orderId , order , item, partialSwap]);
  ...
  return (
    <>
      ...
      />
      <CrossChainSwapModal
        visible={crosschainSwapModalVisible}
        toggle={() => setCrosschainSwapModalVisible(!crosschainSwapModalVisible)}
        loading={loading}
        item={item}
        order={order}
        onApprove={onApprove}
      ...
      >
        ...
        <div style={{ padding: "5px", textAlign: "center" }}>
          <Button2
            onClick={() => {
              if (chainId && item.chainId === order.chainId) {
                setSwapModalVisible(true)
              }
              if (item.chainId !== order.chainId) {
                setCrosschainSwapModalVisible(true)
              }
            }
            }
